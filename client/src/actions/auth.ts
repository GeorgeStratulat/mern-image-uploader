...
export const postLogin = (email: string, password: string) =>
  catchAsync(async (dispatch) => {
    ...
    const res = await axios.post<IUser>(`${axiosURL}/api/auth/login`, {
      email,
      password,
    });
...
export const getLogout = () =>
  catchAsync(async (dispatch) => {
    await axios.get(`${axiosURL}/api/auth/logout`);
    dispatch<LogoutAction>({ type: ActionTypes.logoutUser });
  });

export const registerUser = (form: IRegisterState) =>
  catchAsync(async (dispatch) => {
    const res = await axios.post<IUser>(`${axiosURL}/api/users/register`, form);
